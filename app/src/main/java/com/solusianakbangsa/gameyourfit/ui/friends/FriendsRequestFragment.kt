...
//    TODO : Implement cache store thing (OnSaveInstance thing type beat)
    ...
    private fun retrieveAllRequests() {
        ...
        var reqUsers = FirebaseDatabase.getInstance().reference.child("FriendRequests").child(firebaseUserId).orderByChild("request_type").equalTo("received")
                ...
                for (userReq in mUsers!!) {
                    refUsers.child(userReq).addValueEventListener(object : ValueEventListener {
                        ...
                        override fun onDataChange(snapshot: DataSnapshot) {
                            ...
                            reqAdapter = RequestAdapter(requireActivity(),
    ...
    override fun OnAcceptClick(user: User, position: Int) {
        ...
        AcceptFriend(senderUserId, receiverUserId)
    }

    private fun AcceptFriend(senderUserId: String, receiverUserId: String) {
                                        ...
                                        friendRef.child(receiverUserId)
                                            ...
                                            .addOnCompleteListener{task3->
                                                if (task3.isSuccessful){
                                                    Toast.makeText(requireContext(), "Friend Accepted.", Toast.LENGTH_SHORT).show()
                                                }
                                            }
                                    }else{
